...
The the percent cover of the invasive algae may be influence by the stream substrate size (Figure \ref{fig:algaesediment}, p = 0.0643). 

\begin{figure}[!ht]
<< figure=TRUE, echo=FALSE >>=
boxplot(Algae~Sediment,importupdated, ylab="Algae Abundance (% cover)",xlab="Bed Composition", las=1)
@
\caption{Algae abundance as a function of the bed composition, where course sediments = 1 and fine sediments = 0.}
\label{fig:algaesediment}
\end{figure}

We found no relationship between the percent cover of the invasive algae species on the canopy cover (Figure \ref{fig:canopyalgae}, p = 0.334). 
...
@
\caption{Algae percent cover of red algae as a function of canopy cover}
