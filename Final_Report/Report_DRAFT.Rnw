...
The Santa Ana sucker (\emph{Catostomus santaanae}) is a relatively small fish is extant in a few streams of Southern California. The species has been listed as threatened by the US Fish and Wildlife Service, which has lost over 70\% of its habitat \citep{obrien2011status, usfishandwildlifeservice14}. Besides habitat loss, hydrodomodifications that include the the construction of flood control damns, stream channelization, groundwater extraction, and treated water discharges have a profound effects to river's hydrology and ecology. For example, water temperatures in portions of the Santa Ana river can range from 10-26\textdegree~C while in areas of discharge the elevated temperatues are constantly high due to treated urban discharges \citep{greenfield70}. 

More recently, the abundance of the red algae (\emph{Cosmopogen aeruginosus}) has increased significantly in the Santa Ana River. This may also be contributing to the sucker's decline, but the environmental variables that control the red algae's distribution in the Santa Ana River are currently not well understood. The direct effect this invasive algae has on \emph{C. santaanae} is also poorly constrained. 

In general, the environmental variables that influence \emph{C. santaanae} populations are poorly understood. Better understanding of water temperature, discharge rates, Biochemicial Oxygen Demand (BOD), and red algae distribution could aid in conservation efforts of this federally threantened fish. 
...
<<echo=FALSE>>=
newmetadata <- read.csv("/home/CAMPUS/cmfa2015/Santa Ana Sucker/Data/Data_Tues_2/newmetadata.csv")
@
...
\begin{figure}[!ht]
<< figure=TRUE, echo=FALSE >>=
boxplot(Fish~Section,newmetadata, ylab="Fish",xlab="Time of Day",main ="Time of Day as a predictor of Fish",las=1)
@
