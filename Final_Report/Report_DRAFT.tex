...
Note that *Temp\_x* entries were borrowed with permission from Sophie and Nicole's dataset. We also created a "Site_New" field so that our naming conventions whould be consistant with other teams. We then used rstudio to generate the following descriptive statisitcs:  
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(importupdated\$Temp\_range, importupdated\$Algae, ylab = "{}Algae Abundance (\% cover)"{}, : object 'importupdated' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(importupdated\$Canopy, importupdated\$Algae, ylab = "{}Algae Abundance (\% cover)"{}, : object 'importupdated' not found}}\begin{alltt}
